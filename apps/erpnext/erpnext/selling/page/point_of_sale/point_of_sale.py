...
def get_items(start, page_length, price_list, item_group, pos_profile, search_term="", packaging="false"):
	...
	if items_data:
		...
		item_stock_qty_array = get_stock_availability_all(item_names, warehouse)

		for item in items_data:
			item_code = item.item_code
			item_price = item_prices.get(item_code) or {}

			if item_code in item_stock_qty_array:
				item_stock_qty = item_stock_qty_array[item_code]
			else:
				item_stock_qty = 0
...
def get_items_old(start, page_length, price_list, item_group, pos_profile, search_term=""):
	...
	if items_data:
		...
		for item in items_data:
			if (pos_profile != "MEXICO 2" and frappe.session.user not in ["77050071@shalomcontrol.com","tienda@shalom.com.pe","administrator","48191841@shalomcontrol.com"]) and "MERCHANDISING" in item.item_name:
				continue
			item_code = item.item_code
			item_price = item_prices.get(item_code) or {}
			item_stock_qty = get_stock_availability(item_code, warehouse)
