...
def report_stock():
	...
	values = {"from_date":frappe.form_dict.from_date,"to_date": frappe.form_dict.to_date,"company": "Shalom Empresarial","warehouse": "ALMACEN GENERAL-TIENDA - SE","include_uom":include_uom,"item_code":""}

	columns = get_columns()
	items = get_items(values)
	sl_entries = get_stock_ledger_entries(values, items)
	item_details = get_item_details(items, sl_entries, include_uom)
	opening_row = get_opening_balance(values, columns)
	precision = cint(frappe.db.get_single_value("System Settings", "float_precision"))
