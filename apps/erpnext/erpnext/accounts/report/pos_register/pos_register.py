...
def get_pos_entries(filters, group_by_field, terminalJSON):
	...
	data_entries = frappe.db.sql(
		...
		SELECT 
			...
			p.customer, p.is_return,p.documento, itm.description as item, itm.qty as item_q {select_mop_field} , itm.item_group as grupoItem
		FROM
	...
	for entry in data_entries:
		data.append({
			...
			"item_q": entry.item_q,
			"type_payment": entry.status_comprobante if entry.status_comprobante in ["Pago por PINPAD","QR","Planilla","Link"] else "CONTADO",
		})
...
def get_conditions(filters):
	...
	if filters.get("type_payment"):
		if filters.get("type_payment") in  ["Link", "Pago por PINPAD", "QR","Planilla"]:
			conditions += " AND status_comprobante = %(type_payment)s"
		else:
			conditions += " AND status_comprobante NOT IN ('Link', 'Pago por PINPAD', 'QR','Planilla')"
